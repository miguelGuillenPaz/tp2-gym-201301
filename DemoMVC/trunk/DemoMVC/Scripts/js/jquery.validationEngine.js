 ...
 */
 (function($) {
	 ...
	 var methods = {
			...
			for (var i = 0; i < rules.length; i++) {
				...
				switch (rules[i]) {
					...
					case "ajax":
						...
						break;
                                        case "equalSize":
						errorMsg = methods._equalSize(field, rules, i, options);
						break;
					case "minSize":
		...
		*/
		_maxSize: function(field, rules, i, options) {
			...
			if (len > max) {
                ...
                /**
		* Check the equal size (in characters)
		*
		* @param {jqObject} field
		* @param {Array[String]} rules
		* @param {int} i rules index
		...
		* @return an error string if validation failed
		*/
                _equalSize: function(field, rules, i, options) {
			var equal = rules[i + 1];
			var len = field.val().length;
                        
			if (len != equal) {
				var rule = options.allrules.equalSize;
				return rule.alertText + equal;
			}
		},                
		/**
